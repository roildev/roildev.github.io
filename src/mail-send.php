...
if ($childs_availible === 'on') {
    $childs_availible = 'будут дети. Колличество: ' . $child_quantity . '. Возраст: ' . $child_ages;
} else {
...
$mail->AltBody = $season . ' хотят заказать трансфер - ' . $quantity . ' человек' .  $wait . ' их будет ждать' . $wait_time . 'Им нужно будет проехать примерно: ' . $distance . 'км. ' . 'У них ' . $childs_availible;
